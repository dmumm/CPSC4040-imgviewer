...
int main(int argc, char ** argv)
{
    ...
    userRequest.pImage = inputImage.load(userRequest.imagePath);
    if (! userRequest.pImage) {
        cerr << "ERROR: Image could not be processed.\n";
        exit(EXIT_FAILURE);
    }

    // TODO: debug
    cout << "LD_LIBRARY_PATH:" << getenv("LD_LIBRARY_PATH") << "\n";
    cout << "Program call: " << userRequest.programCall << "\n";


    BasicViewer * viewer = CreateViewer();
    viewer->Init(args, userRequest);
    viewer->setImage(userRequest.pImage);
